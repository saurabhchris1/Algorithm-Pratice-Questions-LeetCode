...
class Solution:
    def fullJustify(self, words, maxWidth: int):
        ...
        for word in words:
            ...
            if len(word) + currWidth + len(currLine) > maxWidth:

                for space in range(maxWidth - currWidth):
                    currLine[space % (len(currLine) - 1 or 1)] += " "
        ...
        remain = " ".join(currLine)
        remain += " " * (maxWidth - len(remain))
        res.append(remain)
