# Given a C++ program, remove comments from it. The program source is an array of strings source where
# source[i] is the ith line of the source code. This represents the result of splitting the original
# source code string by the newline character '\n'.
#
# In C++, there are two types of comments, line comments, and block comments.
#
# The string "//" denotes a line comment, which represents that it and the rest of the characters to
...
class Solution:
    def removeComments(self, source):
        ...
        while right < len(code):
            token = code[right:right + 2]

            if token == "//":
                res.append(code[left:right])
                ...
                while right < len(code) and code[right:right + 2] != "*/":
                    right += 1
                right += 2
                left = right
